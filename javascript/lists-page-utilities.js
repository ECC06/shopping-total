...
//populates the list item in html with it's id, name, date created and date last opened (if it exists)
export function populateListItem(listObj) {
    ...
    const dateCreatedElem = listToPopulate.querySelector("#created-date");
    const lastOpenedElem = listToPopulate.querySelector("#modified-date");
    ...
    dateCreatedElem.innerText = listObj.created;
    ...
    if (listObj.lastOpened) {
        lastOpenedElem.innerText = listObj.lastOpened;
    }
}

// displays the last opened element only if it has a valid date
export function displayLastOpened() {
    listsCont.querySelectorAll("#last-opened").forEach((lastOpenedElem) => {
        if (lastOpenedElem.querySelector("#modified-date").textContent) {
            lastOpenedElem.classList.remove("display-none");
        }
    });
}
...
export function storeReArrangedLists() {
    //a list that is the new order of listsCont.children after the user re-arranges the ui
    const newListsCont = Array.from(listsCont.children);

    const reArrangedItems = [];

    newListsCont.forEach((listElem) => {
        const obj = {};
        obj["id"] = Number(listElem.id);
        obj["listName"] = listElem.querySelector(".list-name").textContent;

        obj["created"] = listElem.querySelector("#created-date").textContent;

        if (listElem.querySelector("#modified-date").textContent) {
            obj["lastOpened"] =
                listElem.querySelector("#created-date").textContent;
        }

        reArrangedItems.push(obj);
    });

    localStorage.setItem("lists", JSON.stringify(reArrangedItems));
}
